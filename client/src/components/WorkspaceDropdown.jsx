    ...
    <div className="relative m-4" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-64 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded shadow-lg top-full left-0">
            ...
            {userMemberships.data.map(({ organization }) => (
              ...
              >
                ...
                {currentWorkspace?.id === organization.id && (
                  <Check className="w-4 h-4 text-ziedet-main dark:text-ziedet-hover shrink-0" />
                )}
              </div>
            ))}
          </div>
          ...
          >
            <p className="flex items-center text-xs gap-2 my-1 w-full text-ziedet-main dark:text-ziedet-hover group-hover:text-ziedet-hover dark:group-hover:text-ziedet-main">
